...
import halsi.rl.agents.ppo_core as core
from halsi.rl.utils.mpi_pytorch import setup_pytorch_for_mpi, sync_params, mpi_avg_grads
from halsi.rl.utils.mpi_tools import mpi_fork, mpi_avg, proc_id, mpi_statistics_scalar, num_procs
import pdb
...
class PPO():
    ...
    def compute_loss_pi(self, data):
        ...
        # True KL using closed form
        with torch.no_grad():
            pi_old = self.ac.pi.dist(obs)
            mu_old, std_old = pi_old.loc, pi_old.scale

        pi_new = self.ac.pi.dist(obs)
        mu_new, std_new = pi_new.loc, pi_new.scale
        ...
        # print("  logp_old[:5] =", logp_old[:5].detach().cpu().numpy())
        # print("  logp_new[:5] =", logp[:5].detach().cpu().numpy())
    ...
    def update(self):
        ...
        for i in range(self.train_pi_iters):
            ...
            kl = pi_info['kl'].detach().cpu().item()
            if kl > 1.5 * self.target_kl:
                print(f"[Residual PPO] Early stopping at step {i} due to reaching max kl ({kl:.6f} > {self.target_kl})")
                break
            loss_pi.backward()
